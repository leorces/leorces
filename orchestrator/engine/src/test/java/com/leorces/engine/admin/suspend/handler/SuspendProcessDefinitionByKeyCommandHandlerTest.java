...
@DisplayName("SuspendProcessDefinitionByKeyCommandHandler Tests")
class SuspendProcessDefinitionByKeyCommandHandlerTest {
    ...
    @InjectMocks
    private SuspendProcessDefinitionByKeyCommandHandler handler;

    @Test
    @DisplayName("should suspend process definition and its processes by key")
    void shouldSuspendByKey() throws Exception {
        // Given
        var batchSize = 10;
        var maxJobs = 1;
        var job = mock(Job.class);
        var input = Map.<String, Object>of();
        var command = new SuspendProcessDefinitionByKeyCommand(PROCESS_DEFINITION_KEY, input);
        ...
        when(processPersistence.suspendByDefinitionKey(PROCESS_DEFINITION_KEY, batchSize))
                .thenReturn(10, 5);
        ...
        verify(definitionPersistence).suspendByKey(PROCESS_DEFINITION_KEY);
        verify(processPersistence, times(2)).suspendByDefinitionKey(PROCESS_DEFINITION_KEY, batchSize);
    }
    ...
    void shouldReturnCorrectCommandType() {
        // When & Then
        assertThat(handler.getCommandType()).isEqualTo(SuspendProcessDefinitionByKeyCommand.class);
    }
