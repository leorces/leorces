...
@DisplayName("ResumeProcessDefinitionByIdCommandHandler Tests")
class ResumeProcessDefinitionByIdCommandHandlerTest {
    ...
    @InjectMocks
    private ResumeProcessDefinitionByIdCommandHandler handler;

    @Test
    @DisplayName("should resume process definition and its processes by id")
    void shouldResumeById() throws Exception {
        // Given
        var batchSize = 10;
        var maxJobs = 1;
        var job = mock(Job.class);
        var input = Map.<String, Object>of();
        var command = new ResumeProcessDefinitionByIdCommand(PROCESS_DEFINITION_KEY, PROCESS_DEFINITION_VERSION, input);
        ...
        when(processPersistence.resumeByDefinitionId(PROCESS_DEFINITION_ID, batchSize))
                .thenReturn(10, 0);
        ...
        verify(definitionPersistence).resumeById(PROCESS_DEFINITION_ID);
        verify(processPersistence, times(2)).resumeByDefinitionId(PROCESS_DEFINITION_ID, batchSize);
    }
    ...
    void shouldReturnCorrectCommandType() {
        // When & Then
        assertThat(handler.getCommandType()).isEqualTo(ResumeProcessDefinitionByIdCommand.class);
    }
